## Build and run
Download-a C# og Code Runner extension í Visual Studio Code
Fara í ClientApp möppuna og keyra ```NPM INSTALL```
Svo bakka í root möppuna og keyra í terminal


```dotnet build```  
```dotnet restore```  
```dotnet run --environment="Development"```  
Að einungis nota dotnet run keyrir Production serverinn virkar ekki fyrir Linux tölvurnar okkar
